...
;; ---------------------------------------------------------------------- ;;
;; INACTIVE -- hand-rolled mode-line and unused functions in new one      ;;
;; ---------------------------------------------------------------------- ;;
...
;; ---------------------------------------------------------------------- ;;
;; ACTIVE -- current mode-line with new function and config options       ;;
;; ---------------------------------------------------------------------- ;;

;; Turn on the column location as character number instead of scroll percentage.
(column-number-mode 1)

;; Everything after the `mode-line-format-right-align' symbol is flushed to
;; the right edge (which edge is set by `mode-line-right-align-edge',
;; default `window').  That symbol is what replaces the manual width
;; arithmetic `simple-mode-line-render' used to do.
;;
;; Note `mode-line-modes' is deliberately NOT used here -- it would drag in
;; every active minor mode.  my-mode-line-minor-modes + my-mode-line-major-mode
;; cover the same ground with the filtering intact.
(setq-default
 mode-line-format
 '("%e"
   mode-line-front-space
